...
- OpenCode Go: read web usage and the Zen balance from the OpenCode Console API and forward the console session cookie, so migrated workspaces report their 5-hour, weekly, and monthly windows and their prepaid balance again instead of failing as expired credentials; workspaces still on the legacy pages are unchanged (fixes #3783). Thanks @kays0x and @NOIPJohnny!
